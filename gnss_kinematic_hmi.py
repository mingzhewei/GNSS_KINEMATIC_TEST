...
APP_TITLE = "GNSS Kinematic HMI"
BASE_DIR = Path(__file__).resolve().parent


def default_output_dir(input_file: str) -> str:
    """当前两个分析器的输出目录规则：输入文件名去掉第一个点后的部分。"""
    return os.path.join(os.path.dirname(input_file), os.path.basename(input_file).split('.')[0])
...
PRODUCTS: dict[str, ProductConfig] = {
    "beiyun": ProductConfig(
        key="beiyun",
        display_name="北云 BEIYUN M21（GNSS/INS 组合惯导）",
        script="gps_kinematic_analyzer_beiyun.py",
        filetypes=(("北云 COM3 数据", "*.dat"), ("所有文件", "*.*")),
        color="#1a5276",
    ),
    "huace": ProductConfig(
        key="huace",
        display_name="华测 HUACE M720（纯 GNSS）",
        script="gps_kinematic_analyzer_huace.py",
        filetypes=(("华测 COM1 数据", "*.dat;*.log"), ("所有文件", "*.*")),
        color="#b03a2e",
    ),
}
...
class GNSSKinematicHMI:
    ...
    @classmethod
    def create_for_test(cls) -> "GNSSKinematicHMI":
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = tk.Label(
            self.root,
            text="统一调度多个 GNSS Kinematic 产品分析脚本",
            font=("微软雅黑", 10),
            bg="#f5f6fa",
            fg="#7f8c8d",
        )
